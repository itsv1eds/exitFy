...
func TestEmbeddedDeprecatedManagerIsRegisteredByInterfaceAndNeverExits(t *testing.T) {
	ctx := service.ContextWith(context.Background(), newEmbeddedDeprecatedManager())
	manager := service.FromContext[deprecated.Manager](ctx)
	if manager == nil {
		t.Fatal("embedded deprecated manager was registered under its concrete type")
	}
