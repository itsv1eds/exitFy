...
// The upstream CLI deprecated manager calls logger.Fatal for impending
// migrations. sing-box's default logger implements Fatal with os.Exit(1),
// which would terminate the host Android application from this C-shared
// library. exitFy owns and validates the generated config, so the embedded
// adapter records no CLI deprecation output and lets normal validation errors
// cross the StartCore boundary instead.
type embeddedDeprecatedManager struct{}

func (embeddedDeprecatedManager) ReportDeprecated(deprecated.Note) {}

func newEmbeddedDeprecatedManager() deprecated.Manager {
	return embeddedDeprecatedManager{}
}
...
func Start(configJSON string) (err error) {
	...
	baseCtx = service.ContextWith(baseCtx, newEmbeddedDeprecatedManager())
