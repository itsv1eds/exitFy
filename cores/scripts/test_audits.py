...
class AuditBoundaryTest(unittest.TestCase):
    ...
    def test_always_on_audit_workflow_has_no_path_filter_or_write_token(self) -> None:
        workflow = (
            audit_public_tree.REPO_ROOT / ".github/workflows/audit-public.yml"
        ...
        self.assertIn(
            'git show "$audit_ref:cores/scripts/audit_public_tree.py"', workflow)
    ...
    def test_release_workflows_reconcile_paginated_drafts_and_exact_tags(self) -> None:
        ...
        self.assertFalse(
            (audit_public_tree.REPO_ROOT / ".github/workflows/release-xray.yml").exists()
        )
        self.assertFalse(
            (audit_public_tree.REPO_ROOT / ".github/workflows/release-singbox.yml").exists()
        )
        for name in ("publish-xray-core-v2.yml", "publish-singbox-core-v2.yml"):
            with self.subTest(name=name):
                workflow = (
                    audit_public_tree.REPO_ROOT / ".github/workflows" / name
                ).read_text(encoding="utf-8")
                ...
                self.assertEqual(build.count("GH_TOKEN:"), 1)
                ...
                self.assertGreaterEqual(
                    build.count(
                        'show "$GITHUB_SHA:cores/scripts/verify_build_inputs.py"'),
                    2,
                )
