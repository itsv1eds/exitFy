...
WORKFLOWS = {
    "xray": ROOT.parent / ".github/workflows/publish-xray-core-v2.yml",
    "sing_box": ROOT.parent / ".github/workflows/publish-singbox-core-v2.yml",
}
...
class ReleaseWorkflowHardeningTest(unittest.TestCase):
    ...
    def test_candidate_handoff_is_immutable_and_verified_before_pin_commit(self) -> None:
        for family, path in WORKFLOWS.items():
            with self.subTest(family=family):
                ...
                self.assertIn('show "$GITHUB_SHA:cores/scripts/candidate_handoff.py"', build)
                ...
                self.assertIn(
                    'show "$GITHUB_SHA:cores/scripts/verify_artifacts.py"',
                    final_verification,
                )
    ...
    def test_upstream_metadata_is_by_commit_and_foreign_pin_children_are_handled(self) -> None:
        ...
        self.assertIn('- "cores/singbox/go.mod"', xray)
        self.assertIn('- "cores/singbox/go.sum"', xray)
        self.assertRegex(sing_box, r'(?m)^      - "cores/go\.mod"$')
        self.assertRegex(sing_box, r'(?m)^      - "cores/go\.sum"$')
        self.assertIn("release_head_state.py", xray)
        self.assertIn("release_head_state.py", sing_box)
        self.assertIn("--foreign-pin cores/singbox/go.mod", xray)
        self.assertIn("--foreign-pin cores/go.mod", sing_box)
    ...
    def test_module_cache_and_source_builder_execute_frozen_code(self) -> None:
        for family, path in WORKFLOWS.items():
            with self.subTest(family=family):
                ...
                self.assertGreaterEqual(
                    build.count('show "$GITHUB_SHA:cores/scripts/verify_build_inputs.py"'),
                ...
                self.assertIn(
                    'show "$GITHUB_SHA:cores/scripts/audit_public_tree.py"',
                    post_test_audit,
                )
        ...
        self.assertEqual(
            sing_box.count(
                'show "$GITHUB_SHA:cores/scripts/build_singbox_source_bundle.py"'
            ),
            2,
        )
        ...
        self.assertEqual(
            sing_box.count(
                'show "$GITHUB_SHA:cores/scripts/audit_singbox_source_bundle.py"'
            ),
            3,
        )
