...
def _audit_index(root: Path, failures: list[str],
...
def _audit_worktree(root: Path, failures: list[str],
                    path_prefixes: list[str] | None = None) -> int:
    count = 0
    for path in public_files(root, path_prefixes):
...
def audit_repository(
    ...
    if head is not None:
        count += _audit_tree(root, head, "HEAD", failures, path_prefixes)
    count += _audit_index(root, failures, path_prefixes)
    count += _audit_worktree(root, failures, path_prefixes)
...
def main() -> None:
    ...
    try:
        ...
        failures, count = audit_repository(
            root, history_base, scan_all_history, args.path_prefixes
        )
