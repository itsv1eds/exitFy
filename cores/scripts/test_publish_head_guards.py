...
def initialize(test: unittest.TestCase) -> tuple[Path, Path, str]:
    ...
    (work / "cores" / "singbox").mkdir(parents=True)
    (work / "cores" / "go.mod").write_text("xray base\n", encoding="utf-8")
    (work / "cores" / "go.sum").write_text("xray sum base\n", encoding="utf-8")
    (work / "cores" / "singbox" / "go.mod").write_text(
        "sing-box base\n", encoding="utf-8")
    (work / "cores" / "singbox" / "go.sum").write_text(
        "sing-box sum base\n", encoding="utf-8")
    run_git(work, "add", ".")
...
class PublishHeadGuardTest(unittest.TestCase):
    def exercise_family(self, workflow_name: str, own_prefix: str, foreign: str) -> None:
        workflow = guard_script(ROOT.parent / ".github" / "workflows" / workflow_name)
    ...
    def test_xray_guard_accepts_exact_own_or_one_foreign_pin_child(self) -> None:
        self.exercise_family(
            "publish-xray-core-v2.yml", "cores", "cores/singbox/go.mod"
        )

    def test_singbox_guard_accepts_exact_own_or_one_foreign_pin_child(self) -> None:
        self.exercise_family(
            "publish-singbox-core-v2.yml", "cores/singbox", "cores/go.mod"
        )
